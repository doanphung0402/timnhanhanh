...
const Search = () => {
    ...
    const [isSearchAddr,setIsSearchAddr] = useState(false) 
    ...
    const handleSubmitProvince = (e, query) =>{
        e.stopPropagation()
    ...
    const handleSearch = () => {
        const queryCodes = Object.entries(queries).filter(item => item[0].includes('Number')||item[0].includes('Name') ||item[0].includes('Id')|| item[0].includes('Code')).filter(item => item[1])
        let queryCodesObj = {}
    ...
    return (
        <>
            <div className='p-[10px]  my-3 bg-[#FFFFFF] rounded-lg flex-col lg:flex-row flex items-center justify-around gap-2' >
                ...
                <span onClick={() => {handleShowModal(provinces, 'province', 'Toàn quốc'); setIsSearchAddr(true)}} className='cursor-pointer flex-1'>
                    <SearchItem IconBefore={<HiOutlineLocationMarker />} IconAfter={<BsChevronRight color='rgb(156, 163, 175)' />} text={`${queries.province===undefined?"Toàn quốc":queries.province} ${queries.province?",":""} ${queries.districtName===undefined?"":queries.districtName}`} defaultText={'Toàn quốc'} />
                </span>
                ...
                <button
                    ...
                    className='outline-none py-2 px-4 flex-1 bg-[#ED570E] text-[13.3px] flex items-center justify-center gap-2 text-white font-medium'
                >
                    <FiSearch />
                    Tìm kiếm
                </button>
            ...
            {isShowModal  && <Modal
                ...
                handleSubmitProvince ={handleSubmitProvince}
                queries={queries}
                arrMinMax={arrMinMax}
                content={content}
                name={name}
